(function ($) {
    ...
    $.fn.duration_picker = function (options) {
        ...
        this.each(function (i, e) {
            ...
            if (!disabled) {
                var picker = $('<div class="card left"><div class="card-content"></div></div>').css('width', 110);
                var num_input = $('<input style="width: 40px;" type="number" min="0" value="0">');
                var day_input = num_input.clone().change(picker_changed);
                $('<div class=""> ' + langs[settings.lang]['days'] + '</div>').prepend(day_input).appendTo(picker);
                var hour_input = num_input.clone().attr('max', 23).change(picker_changed);
                $('<div class=""> ' + langs[settings.lang]['hours'] + '</div>').prepend(hour_input).appendTo(picker);
                var minute_input = num_input.clone().attr('max', 59).change(picker_changed);
                $('<div class=""> ' + langs[settings.lang]['minutes'] + '</div>').prepend(minute_input).appendTo(picker);
                var second_input = num_input.clone().attr('max', 59).change(picker_changed);
                $('<div class=""> ' + langs[settings.lang]['seconds'] + '</div>').prepend(second_input).appendTo(picker);
                e.after(picker);
                picker.hide();
